...
import com.liteflow.modules.analytics.dao.RevenueReportDAO;
...
public class RevenueReportService {
    ...
    public JSONObject generateReport(LocalDate startDate, LocalDate endDate) {
        ...
        try {
            ...
            // 🆕 Get Room Occupancy
            report.put("occupancyData", generateOccupancyData());
     ...
     */
    private JSONObject generateOccupancyData() {
        JSONObject data = new JSONObject();
        try {
            Map<String, Long> occupancy = reportDAO.getRoomOccupancySummary();
            for (Map.Entry<String, Long> entry : occupancy.entrySet()) {
                data.put(entry.getKey(), entry.getValue());
            }
        } catch (Exception e) {
            System.err.println("❌ Error generating occupancy data: " + e.getMessage());
        }
        return data;
    }
}
